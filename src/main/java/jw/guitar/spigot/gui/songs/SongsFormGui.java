package jw.guitar.spigot.gui.songs;
...
import jw.guitar.core.data.songs.Song;
import jw.guitar.core.data.songs.SongsRepository;
import jw.guitar.core.factory.ButtonsFactory;
import jw.guitar.spigot.gui.ChordPickerGui;
...
public class SongsFormGui extends ChestUI {
    ...
    protected void onInitialize() {
        ...
        ButtonUI.builder()
                ...
                .setLocation(3, 6)
                .setDescription(FluentApi.translator().get("gui.piano.rename.desc"))
    ...
    private void refreshChords()
    {
        ...
        for(var i =0;i<INVENTORY_WIDTH;i++)
        {
            ...
            {
                var chord = chordService.getAll(chordName);
                button.setTitlePrimary(chord.fullName());
                button.setCustomMaterial(chord.getItemStack().getType(), chord.getCustomId());
            }
        }
        ...
        ButtonUI.builder()
                ...
                .setOnClick((player, button) ->
                {
                    if (isInsert) {
                        ...
                        FluentApi.logger().log("inserting", result);
                    } else {
                        var result = songsRepository.update(song.getUuid(), song);
                        FluentApi.logger().log("updating", result);
                    }
                    openParent();
                })
